...
### حلول سريعة
1. **تشغيل MongoDB محليًا** (Windows/Linux/macOS) ثم إعادة تشغيل السيرفر.
2. **استخدام MongoDB Atlas**:
   - أنشئ Cluster مجاني.
   - انسخ Connection String.
   - ضع الرابط في `backend/.env` داخل `MONGO_URI`.
3. تأكد أن IP مسموح في Atlas (Network Access) وأن اسم المستخدم/كلمة المرور صحيحان.

### تحسين في هذا المشروع
- تمت إضافة إعادة محاولة اتصال تلقائية بقاعدة البيانات عبر:
  - `DB_MAX_RETRIES`
  - `DB_RETRY_DELAY_MS`
- إضافة مسار `/` ورسالة واضحة من API، ومسار `/api/health` يُرجع حالة اتصال قاعدة البيانات.
